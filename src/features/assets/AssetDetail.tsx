...
import { formatBytes, formatDate, formatDuration, friendlyError, statusLabel } from '@/lib/format';
      ...
      <div className="panel__scroll">
        ...
        {asset && (
          <div className="panel__body">
            ...
            <div className="status-steps" role="group" aria-labelledby="status-label">
              {STATUSES.map((status, i) => (
                <div key={status} className="status-step">
                  <button
                    className={`status-step__btn status-step__btn--${status}`}
                    disabled={saving || status === asset.status}
                    aria-pressed={status === asset.status}
                    title={`Step ${i + 1} of ${STATUSES.length}`}
                    onClick={() => setStatus(status)}
                  >
                    <span className="status-step__num">{i + 1}</span>
                    {statusLabel(status)}
                  </button>
                </div>
              ))}
